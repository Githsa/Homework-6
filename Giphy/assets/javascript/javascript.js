 ...
 */
 ...
 var gif = {
 	...
 	// Main topics that holds all topics
 	topics: ["Hockey", "Basketball", "Baseball", "Soccer", "Football", "Surfing"],
 	...
 	// Hold new topics added in another topics so that items may be removed in the future when user selects "Clear Data"
 	newtopics: [],

 	/**
 	 * Populate default buttons on page load, from topics
	 * @param {topics} items The topics of topics
	 ...
	 */
 	generateDefaultButtons: function(items, action) {
 		...
 		// Loop through the topics of topics and generate the default button list
 		//gif.topics.forEach(function(item, index, arr) {
 	...
 	generateNewButton: function(topic) {
 		...
 		if(gif.topics.indexOf(topic) != -1) {
 			this.showModal("That topic already exists! Please choose another one.");
 			return false;
 		}

 		// Unshift (push) new topic onto beginning of main topics (so that page reload maintains same order of topic topics as 
 		// display after append topic instruction). Also append to temp topics (will be used later for removing new topics)
 		gif.topics.unshift(topic);
 		gif.newtopics.push(topic);

 		// Save to session storage so that buttons generated by user may be saved
 		sessionStorage.setItem("topics", gif.topics);
 	...
 	generateGifs: function(topic) {
 		...
 		$.ajax({
 			...
 			// Loop through returnData.  Note:  is an object.  Must reference 'data' to access the topics
 	...
 	/**
 	 * Clear the session storage (localStorage) data == topics of topics
 	 * @param N/A
 	...
 	clearStorage: function() {
 		...
 		if(localStorage.getItem("topics") == null) {
 			...
 			localStorage.clear("topics");
 			// Show modal msg to user confirming that data was cleared
 			this.showModal("You successfully cleared the session data!");
 			// Remove new items from original topic topics
 			gif.restoretopics();
 			// Return true so that generateDefaultButtons will only run after topics clearing has completed (else dupe error may trigger)
 			return true;
 		}
 	},

 	/**
 	 * Remove items in newtopics from original topics for restoring original 
 	 * topics list after "Clear Data" button is clicked by user.
 	 * @param N/A
 	 * @return N/A
 	 */
 	restoretopics: function() {
 		// Run through each item in newtopics, removing matches found in topics (every item will match ALWAYS)
 		gif.newtopics.forEach(function(item, index, arr) {
 			var index = gif.topics.indexOf(item);
 			if(index != -1) {
 				gif.topics.splice(index, 1);
 			}
 		});
 	},
...
// Main program executable code
 $(document).ready(function() {
 	...
 	var items = localStorage.getItem("topics");
 	if(items != null) {
 		items = JSON.parse(items);
 		var action = null;
 		gif.generateDefaultButtons(items, action);
 	} else {
 		var items = gif.topics;
 		var action = null;
 		gif.generateDefaultButtons(items, action);
 	}
 	...
 	$('span.search-icon').on('click', function(e) {
 		...
 		// Generate a new button with the user's input, and add topic to default and new topic topicss
 		gif.generateNewButton(text);

 		// Save entire topics to local storage
 		localStorage.setItem("topics", JSON.stringify(gif.topics));
 	...
 	// When user clicks on "Clear Data" button, clear the localStorage topic topics if it exists
 	$('button.clear-btn').on('click', function() {
 		...
 		if(gif.clearStorage()) { 
 			// Note: need to clear out the new topics items so that the error "Topic already exists" does not show
 			gif.generateDefaultButtons(gif.topics, action);
 		}
 	});
